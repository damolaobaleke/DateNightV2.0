...
package com.datenight_immersia_ltd.modelfirestore;
...
public class DateNightAppData {
    ...
    // Util data
    private String TAG = "RetrieveUserAndExperienceData";

    public DateNightAppData (String currentUserId, Context launchContext){
        this.currentUserId = currentUserId;
        if (currentUserId != null){
            dbFirestore = FirebaseFirestore.getInstance();
            dbFirestore.collection(DatabaseConstants.USER_DATA_NODE).document(
                    currentUserId).get().addOnSuccessListener(userDocumentSnapshot -> {
